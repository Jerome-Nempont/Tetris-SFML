...
//Chercher la texture selon sa forme
...
void Joueur::bougerTetromino(int dir) //Gauche/Droite
{
    int dern_el=m_vecTetrominos.size()-1;
...
void Joueur::rotationTetromino(int sensR)//Rotation du tetromino
{
    ...
    if(formeTetro!=FORME_O)
    {
        ...
        while(comptt<4)
        {
            ...
            if(sensR==ROTATION_G) //Rotation gauche selon le centre de rotation
            {
                xf=centreRot.x+(yd*1);
                yf=centreRot.y+(-xd*1);
            }
            else //Rotation droite selon le centre de rotation
            {
                xf=centreRot.x+(-yd*1);
                yf=centreRot.y+(xd*1);
            }
